...
echo '(module'
grep '^  (type'                  "$input_wat"
grep '^  (import "env".*(memory' "$input_wat" | sed 's/  (import "env" \("[a-z1-9_]*"\) (memory (;[0-9]*;) \([0-9]*\)))/  (memory (export \1) \2)/'
grep '^  (import "env".*(func'   "$input_wat" | sed 's/  (import "env" \("[a-z1-9_]*"\) (func \(\$[a-z1-9_]*\) \((type [0-9]*)\)))/  (func \2 (export \1) \3 (phost . \2))/'
echo ')'
echo ''
echo '(register "env")'
